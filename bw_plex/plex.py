...
from bw_plex.misc import (
    analyzer,
    choose,
    find_next,
    find_offset_ffmpeg,
    find_theme_start_end,
    get_pms,
    get_hashtable,
    has_recap,
    to_sec,
...
def shutdown_handler(sig, stack):  # pragma: no cover
    LOG.info("Got a signal %s doing some " "cleanup before shutting down", sig)
...
def process_to_db(
    ...
    if vid is None and media.TYPE == "episode":
        vid = convert_and_trim(
            check_file_access(media),
            fs=11025,
    ...
    if (
        media.TYPE == "episode"
        and CONFIG["tv"].get("check_credits") is True
        and credits_start is None
        and credits_end is None
    ):

        dur = media.duration / 1000 - CONFIG["tv"].get("check_credits_sec", 120)
        credits_start, credits_end = find_credits(
            check_file_access(media), offset=dur, check=-1
        )
    ...
    with session_scope() as se:
        try:
            ...
            if media.TYPE == "episode":
                ...
                media.TYPE == "movie"
                and CONFIG.get("movie", {}).get("create_chapters", False) is True
            ):
                p = Processed(
                    title=media.title,
                    type=media.TYPE,
...
@click.option(
    "--servername", "-s", default=None, help="The server you want to monitor."
)
...
@click.option(
    ...
    default=False,
    is_flag=True,
    help="Enable this to allow insecure connections to PMS",
)
@click.option(
    "--default_folder",
    "-df",
    default=None,
    help="Override for the default folder, typically used by dockers.",
)
@click.option(
    "--nice", "-n", default=None, type=int, help="Set niceness of the process."
)
def cli(
    debug,
    username,
...
def check_db(client_name, skip_done):  # pragma: no cover
    ...
    with session_scope() as se:
        ...
        for item in sorted(items, key=lambda k: k.ratingKey):
            ...
            click.echo(
                "theme_start %s theme_end %s ffmpeg_end %s"
            ...
            if item.theme_end != -1 and item.type == "episode":
                ...
                if (
                    not skip_done and item.correct_theme_start
                ) or not item.correct_theme_start:

                    click.echo(
                        "Found theme_start at %s %s theme_end %s %s"
                        % (
                            item.theme_start,
                ...
                if (
                    not skip_done and item.correct_theme_end
                ) or not item.correct_theme_end:

                    client.playMedia(media, offset=item.theme_end * 1000)
                    end_match = click.prompt(
                        "Was theme_end at %s correct? [y or MM:SS]" % item.theme_end_str
                    )
                    if end_match:
            ...
            if item.ffmpeg_end:
                if (not skip_done and item.correct_ffmpeg) or not item.correct_ffmpeg:
                    click.echo(
                        "Found ffmpeg_end at sec %s time %s"
            ...
            if item.credits_start and item.credits_start != 1:
                if (
                    not skip_done and item.correct_credits_start
                ) or not item.correct_credits_start:
                    click.echo(
                        "Found credits start as sec %s time %s"
...
@click.option(
    "-s", "--sample", default=0, help="Process N episodes of all shows.", type=int
)
@click.option("-t", "--threads", help="Threads to uses", default=1, type=int)
@click.option(
    "-sd",
    "--skip_done",
    help="Skip media items that exist in the db",
    default=True,
    is_flag=True,
)
        ...
        for media in medias:
            if media.TYPE == "show":
                ...
                eps = choose(
                    "Select episodes",
                    eps,
                    lambda x: "%s %s" % (x._prettyfilename(), x.title),
                )
                all_items += eps
            else:
...
@click.option(
    "-dev", default=7, help="Accepted deviation between audio and video", type=int
)
...
@click.option(
    "-t",
    default="scene marker",
    type=click.Choice(["cut", "scene marker", "mute", "commercial break"]),
...
def add_hash_frame(name, dur, sample):  # pragma: no cover
    ...
    def to_db(media):
        ...
        imgz = []
        for imghash, _, pos in hash_file(
            check_file_access(media), frame_range=False, end=dur
        ):
            img = Images(
                ratingKey=media.ratingKey,
                hex=str(imghash),
...
def test_hashing_visual(name, conf):  # pragma: no cover
    ...
    assert (
        len(all_items) == 1
    ), "visulize_intro_from_hashes only works on one file at the time"
    ...
    def find_intro_from_hexes_in_db(item):
        ...
        with session_scope() as se:
            eps = se.execute(
                "select count(distinct ratingKey) from images where grandparentRatingKey = %s and parentRatingKey = %s"
                % (item.grandparentRatingKey, item.parentRatingKey)
            )
            eps = list(eps)[0][0]
            LOG.debug(
                "%s season %s has %s episodes",
                item.grandparentTitle,
                item.parentIndex,
                eps,
            )
            stuff = se.execute(
                "select * from images where grandparentRatingKey = %s and parentRatingKey = %s"
                % (item.grandparentRatingKey, item.parentRatingKey)
            )
            stuff = list(stuff)
        ...
        LOG.debug(
            "Found %s hashes that are in %s percent of the episodes (%s) in this season",
            len(new_hex),
            eps,
            100 * conf,
        )
...
def add_ref_frame(fp, t, tvdbid, timestamp, gui):  # pragma: no cover
    ...
    with session_scope() as se:
        try:
            ...
            se.add(frm)
            LOG.debug(
                "Added %s to Reference_Frame table hex %s tvdbid %s",
                fp,
                frames_hex,
                tvdbid,
            )
...
@click.option(
    ...
    "--type",
    default=None,
    type=click.Choice(["manual", "tvtunes", "plex", "youtube", "all"]),
)
...
def manually_correct_theme(
    name, url, type, rk, just_theme, remove_old_theme
):  # pragma: no cover
    ...
    if rk == "auto":
        ...
        items = choose(
            "Select correct show", items, lambda x: "%s %s" % (x.title, x.TYPE)
        )
        if items:
...
@click.option(
    "-d",
    "--directory",
    help="What directory you want to scan for themes.",
    default=None,
)
def add_theme_to_hashtable(threads, directory):
    ...
    LOG.debug(
        "Creating hashtable, this might take a while.. adding %s files", len(all_files)
    )
...
@click.option(
    "-f",
    "--format",
    type=click.Choice(["txt", "html", "json", "yaml", "dbf", "csv"]),
    default="txt",
)
...
def client_action(offset=None, sessionkey=None, action="jump"):  # pragma: no cover
    ...
    called = time.time()
    LOG.info(
        "Called client_action with %s %s %s %s",
        offset,
        to_time(offset),
        sessionkey,
        action,
    ...
    for media in PMS.sessions():
        ...
        if sessionkey and int(sessionkey) == media.sessionKey:
            ...
            if client.platform == "Chromecast" and client.local:
                ...
                # it takes like 5 sec.
                pc, cast = log_exception(
                    get_chromecast_player(client.address, "Chromecast")
                )
                if pc and cast:
                    ...
                    LOG.debug(
                        "The client was a chromecast but we couldnt get a plex controller "
            ...
            if action != "stop":
                if ignore_ratingkey(
                    media, CONFIG["general"].get("ignore_intro_ratingkeys")
                ):
                    LOG.info(
                ...
                LOG.debug(
                    "calculated_offset %s %s"
                    % (calculated_offset, calculated_offset / 1000)
                )
                ...
                if not ignore_ratingkey(
                    media, CONFIG["general"].get("ignore_outro_ratingkeys")
                ):
                    ...
                    LOG.debug(
                        "Stopped playback on %s and marked %s as watched.",
                        client.title,
                        media._prettyfilename(),
                    )
...
def task(item, sessionkey):
    ...
    if media.TYPE == "episode":
        LOG.debug(
            "Download the first 10 minutes of %s as .wav", media._prettyfilename()
        )
        vid = convert_and_trim(
            check_file_access(media),
            fs=11025,
...
def jump(item, sessionkey, sec=None, action=None):  # pragma: no cover
    ...
    if action is None and sessionkey not in JUMP_LIST:
        LOG.debug(
            "Called jump with %s %s %s %s", item.prettyname, sessionkey, sec, action
        )
...
def check(data):
    if data.get("type") == "playing" and data.get("PlaySessionStateNotification"):
        ...
        with session_scope() as se:
            try:
                ...
                if item:
                    bt = best_time(item)
                    LOG.debug(
                        "Found %s theme start %s, theme end %s, ffmpeg_end %s progress %s fake_progress %s"
                    ...
                    if (
                        item.type == "episode"
                        and CONFIG["tv"].get("check_credits") is True
                        and CONFIG["tv"].get("check_credits_action") in ("stop", "seek")
                        or item.type == "movie"
                        and CONFIG["movie"].get("check_credits") is True
                        and CONFIG["movie"].get("check_credits_action") == "stop"
                    ):
                        ...
                        if (
                            ...
                            and fake_progress >= item.credits_start
                        ):
                            ...
                            if item.type == "episode":
                                ...
                                if act == "seek":
                                    ...
                                    act_to_time = item.credits_start + CONFIG["tv"].get(
                                        "credits_delay", 0
                                    )
                            else:
                                act = CONFIG["movie"].get("check_credits_action")
                                act_to_time = item.credits_start + CONFIG["movie"].get(
                                    "credits_delay", 0
                                )
                    ...
                    if mode == "skip_only_theme":
                        ...
                        if (
                            item.type == "episode"
                            and item.correct_theme_end
                            and item.correct_theme_start
                        ):
                            if (
                                fake_progress > item.correct_theme_start
                                and fake_progress < item.correct_theme_end
                            ):
                                LOG.debug(
                            ...
                            item.type == "episode"
                            and item.theme_end
                            and item.theme_start
                        ):
                            if (
                                fake_progress > item.theme_start
                                and fake_progress < item.theme_end
                            ):
                                LOG.debug(
        ...
        if (
            ...
            if (
                ...
                LOG.debug(
                    "Didn't start to process %s is process_recently_added is disabled",
                    title,
                )
                return
        ...
        elif metadata_type in (1, 4) and state == 9 and metadata_state == "deleted":
            ...
            if (
                ...
                LOG.debug(
                    "Didn't start to process %s is process_deleted is disabled for",
                    title,
                )
                return
            ...
            with session_scope() as se:
                try:
                    ...
                    se.delete(item)
                    LOG.debug(
                        "%s was deleted from %s and from media.db",
                        title,
                        PMS.friendlyName,
                    )
                except NoResultFound:
...
def watch():  # pragma: no cover
    ...
    ws_url = (
        PMS.url("/:/websockets/notifications", includeToken=True)
        .replace("http", "ws")
        .replace("https", "wss")
    )
    ws = WebSocket(ws_url)
...
def set_manual_theme_time(
    showname, season, episode, type, start, end
):  # pragma: no cover
    ...
    if result:
        ...
        if ep:
            with session_scope() as se:
                ...
                LOG.debug(
                    "Set correct_time %s for %s to start %s end %s",
                    type,
                    ep._prettyfilename(),
                    start,
                    end,
